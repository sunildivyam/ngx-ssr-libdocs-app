...
import {
  ...
  LibInfo,
  LibsInfo
} from '@annuadvent/ngx-lib-docs/docs-common';
...
export class LibdocsMetaService {
  ...
  public setPageMeta(
    ...
    const pageMeta: MetaInfo = {
      ...
      description:
        description || appMetaInfo.description.replace('{{libNames}}', ''),
      keywords: keywords?.length ? keywords.join(', ') : appMetaInfo.keywords,
      'article:tag': keywords?.length
        ? keywords.join(', ')
        : appMetaInfo.keywords,
  ...
  public setLibsHomePageMeta(libsInfo: LibsInfo): MetaInfo {
    ...
    return this.setPageMeta(
      ...
      libHomeMetaInfo.description.replace(
        '{{libNames}}',
        libsInfoArr.map((li) => li?.name).join(', ')
      ),
      [
        ...libHomeMetaInfo.keywords.split(','),
        ...libsInfoArr.map((li) => li?.name || '')
      ]
    );
  }
  ...
  public setAssetTypeInfoPageMeta(
    assetType: string,
    libInfo: LibInfo
  ): MetaInfo {
    return this.setPageMeta(
